...
int main(int argc, char **argv)
{
    ...
    dumpfile = pcap_dump_open(descr, "q3_output.pcap");
    if (dumpfile == NULL) {
        fprintf(stderr, "Error opening output file for pcap: %s\n", pcap_geterr(descr));
        exit(1);
    }


    // Loop through packets and call packetHandler
    pcap_loop(descr, MAX_PACKETS, packetHandler, (u_char *)dumpfile);
